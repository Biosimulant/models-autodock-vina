...
A single docking job through the `vina-autodock-vina-docking-predictor` wrapper, against the checked-in `1iep` `PDBQT` pair under `labs/vina-autodock-vina-docking-predictor/models/core/data/1iep/`. The full input set (search box, exhaustiveness, scoring) is in [config.yaml](config.yaml).
