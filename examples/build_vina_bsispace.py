...
MODEL_DIR = (
    ...
    / "models"
    / "vina-autodock-vina-docking-predictor"
)
EXAMPLE_CONFIG = REPO_ROOT / "models" / "models-autodock-vina" / "examples" / "vina-minimal" / "config.yaml"
DEFAULT_OUTPUT = Path("/Users/demi/Desktop/AutoDock_Vina_Remote_CPU_Example.bsispace")
...
def parse_example_inputs(config_path: Path) -> tuple[str, str, dict[str, Any]]:
    ...
    receptor_pdbqt_path = str(inputs.get("receptor_pdbqt_path") or "").strip()
    ligand_pdbqt_path = str(inputs.get("ligand_pdbqt_path") or "").strip()
    run_options = inputs.get("run_options") if isinstance(inputs.get("run_options"), dict) else {}
    if not receptor_pdbqt_path or not ligand_pdbqt_path:
        raise RuntimeError(
            f"Could not parse receptor_pdbqt_path and ligand_pdbqt_path from {config_path}"
        )
    return receptor_pdbqt_path, ligand_pdbqt_path, dict(run_options)
...
def choose_remote_sizes(catalog: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    cpu_sizes = [
        ...
        if not size.get("gpu_type") and int(size.get("gpu_count") or 0) == 0
    ]
    if cpu_sizes:
        cpu_sizes.sort(
            key=lambda size: (
        ...
        return cpu_sizes
    raise RuntimeError("No affordable CPU remote sizes are available for this account.")
    ...
    for remote_size in remote_sizes:
        try:
            ...
            if _is_remote_size_capacity_error(exc):
                continue
            raise
    raise RuntimeError(f"Could not create a remote run on any affordable CPU size: {last_error}")
...
def assert_results_shape(results_payload: dict[str, Any]) -> None:
    ...
    for key in ("pose_summary", "docking_summary", "structure_artifacts"):
...
def build_manifest(
    ...
    return {
        "schema_version": "2.0",
        "title": "AutoDock Vina Remote CPU Example",
        "description": "Portable single-model AutoDock Vina lab intended for remote CPU execution and inline 3D docked-complex visualisation.",
        "models": [
            {
                "alias": "vina",
                ...
                "parameters": {
                    ...
                    "default_receptor_pdbqt_path": receptor_pdbqt_path,
                    "default_ligand_pdbqt_path": ligand_pdbqt_path,
        ...
        "scientific_context": {
            "question": "Can AutoDock Vina dock a prepared ligand into a prepared receptor remotely and return a renderable 3D structure inside the desktop app?",
            "mode": "native-vina",
            "assumptions": [
                "Remote runs should use CPU-backed execution.",
                "The receptor and ligand are already prepared as PDBQT inputs.",
            ],
            "expected_observables": [
                "Ranked pose affinity values from AutoDock Vina.",
                "A docked top-pose summary plus config metadata.",
...
def build_layout() -> dict[str, Any]:
    return {
        "nodes": [
            {"id": "vina", "type": "model", "x": 180, "y": 120},
        ],
...
def mirror_and_wait_for_remote_results(
    ...
    while time.time() < deadline:
        ...
        time.sleep(poll_seconds)
    raise TimeoutError(
        f"Remote run {remote_run_id} did not finish within {timeout_seconds} seconds. Last state: {last_remote_state}"
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Build and validate a portable AutoDock Vina .bsispace package.")
    ...
    receptor_pdbqt_path, ligand_pdbqt_path, run_options = parse_example_inputs(EXAMPLE_CONFIG)
    build_root = Path(tempfile.mkdtemp(prefix="vina-bsispace-build-"))
    export_dir = build_root / "export"
    export_dir.mkdir(parents=True, exist_ok=True)
    package_tmp_path = export_dir / "AutoDock_Vina_Remote_CPU_Example.bsispace"
    ...
    try:
        created_space = run_cli(
            ...
            {
                "title": "AutoDock Vina Remote CPU Example",
                "description": "Portable remote CPU AutoDock Vina example lab",
            },
        )
        ...
        if not args.skip_remote_validation:
            ...
            cpu_sizes = choose_remote_sizes(catalog)
        ...
        summary = {
            ...
            "space_title": "AutoDock Vina Remote CPU Example",
